...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/interact':
            ...
            """
            @todo Identify why do we need to call parley to change personas
            Load random persona for the agent by calling world.parley.
            """
            SHARED['agent'].reset()
            SHARED['dummy_agent'].reset()
            SHARED['world'] = create_task(SHARED.get('opt'), [SHARED['dummy_agent'], SHARED['agent']])
            SHARED['world'].parley()
            print(SHARED['world'].display())
...
def setup_interactive(shared):
    ...
    parser.add_argument('-t', '--task', type=str, default="blended_skill_talk", help='task to use')
    SHARED['opt'] = parser.parse_args(print_args=False)

    """
    Create model and assign it to the specified task.
    The only use of the dummy agent is to satisfy the 
    requirement of 2 agents for blended_skill_talk task
    """
    agent = create_agent(SHARED.get('opt'), requireModelExists=True)
    dummy_agent = create_agent(SHARED.get('opt'), requireModelExists=True)

    SHARED['agent'] = agent
    SHARED['dummy_agent'] = dummy_agent
    SHARED['world'] = create_task(SHARED.get('opt'), [SHARED['dummy_agent'], SHARED['agent']])
